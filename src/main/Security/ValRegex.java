...
public class ValRegex {
    ...
    public static final Pattern orgNamePattern = Pattern.compile(
        "^[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,.'-0-9]{1,200}$");

    public static final Pattern zipCodePattern = Pattern.compile("^\\d{5}(-\\d{4})?$");

    public static final Pattern usStatePattern = Pattern.compile("^(AA|AE|AP|AL|AK|AS|AZ|AR|CA|CO"
    ...
    public static final Pattern cityPattern = Pattern.compile("^[a-zA-Z]+(?:(?:\\s+|-)[a-zA-Z]+)"
        + "*$");

    public static final Pattern phoneNumberPattern = Pattern.compile("^\\s*(?:\\+?(\\d{1,3}))?[-."
        + " (]*(\\d{3})[-. )]*(\\d{3})[-. ]*(\\d{4})(?: *x(\\d+))?\\s*$");
}
