...
function PreviewBooleanCell(props: CellProps) {
  ...
  return (
    <div className="tw-flex tw-flex-wrap tw-items-center tw-gap-6">
      <label className={`tw-inline-flex tw-items-center tw-gap-3 tw-text-lg ${!enabled ? 'tw-opacity-50' : 'tw-cursor-pointer'}`}>
        <input
          type="radio"
          name={id}
          checked={checked}
          ...
          disabled={!enabled}
          className="tw-h-5 tw-w-5 tw-border-gray-300 tw-text-blue-600 focus:tw-ring-2 focus:tw-ring-blue-500"
        />
        Yes
      </label>
      <label className={`tw-inline-flex tw-items-center tw-gap-3 tw-text-lg ${!enabled ? 'tw-opacity-50' : 'tw-cursor-pointer'}`}>
