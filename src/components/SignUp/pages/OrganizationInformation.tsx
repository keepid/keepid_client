...
export class OrganizationInformation extends Component<Props, State, {}> {
  ...
  validateOrgName = async (): Promise<void> => {
    ...
    if (isValidOrgName(orgName)) {
      await new Promise((resolve) =>
        this.setState({ orgNameValidator: 'true' }, resolve));
    } else {
      await new Promise((resolve) =>
        this.setState({ orgNameValidator: 'false' }, resolve));
    }
  };
  ...
  validateOrgWebsite = async (): Promise<void> => {
    ...
    if (orgWebsite !== '' && isValidOrgWebsite(this.addHttp(orgWebsite))) {
      await new Promise((resolve) =>
        this.setState({ orgWebsiteValidator: 'true' }, resolve));
    } else {
      await new Promise((resolve) =>
        this.setState({ orgWebsiteValidator: 'false' }, resolve));
    }
  };
  ...
  validateEIN = async (): Promise<void> => {
    ...
    if (isValidEIN(ein)) {
      await new Promise((resolve) =>
        this.setState({ einValidator: 'true' }, resolve));
    } else {
      await new Promise((resolve) =>
        this.setState({ einValidator: 'false' }, resolve));
    }
  };
  ...
  validateOrgAddress = async (): Promise<void> => {
    ...
    if (isValidAddress(orgAddress)) {
      await new Promise((resolve) =>
        this.setState({ orgAddressValidator: 'true' }, resolve));
    } else {
      await new Promise((resolve) =>
        this.setState({ orgAddressValidator: 'false' }, resolve));
    }
  };
  ...
  validateOrgCity = async (): Promise<void> => {
    ...
    if (isValidCity(orgCity)) {
      await new Promise((resolve) =>
        this.setState({ orgCityValidator: 'true' }, resolve));
    } else {
      await new Promise((resolve) =>
        this.setState({ orgCityValidator: 'false' }, resolve));
    }
  };
  ...
  validateOrgState = async (): Promise<void> => {
    ...
    if (isValidUSState(orgState)) {
      await new Promise((resolve) =>
        this.setState({ orgStateValidator: 'true' }, resolve));
    } else {
      await new Promise((resolve) =>
        this.setState({ orgStateValidator: 'false' }, resolve));
    }
  };
  ...
  validateOrgZipcode = async (): Promise<void> => {
    ...
    if (isValidZipCode(orgZipcode)) {
      await new Promise((resolve) =>
        this.setState({ orgZipcodeValidator: 'true' }, resolve));
    } else {
      await new Promise((resolve) =>
        this.setState({ orgZipcodeValidator: 'false' }, resolve));
    }
  };
  ...
  validateOrgPhoneNumber = async (): Promise<void> => {
    ...
    if (isValidPhoneNumber(orgPhoneNumber)) {
      await new Promise((resolve) =>
        this.setState({ orgPhoneNumberValidator: 'true' }, resolve));
    } else {
      await new Promise((resolve) =>
        this.setState({ orgPhoneNumberValidator: 'false' }, resolve));
    }
  };
  ...
  validateOrgEmail = async (): Promise<void> => {
    ...
    if (isValidEmail(orgEmail)) {
      await new Promise((resolve) =>
        this.setState({ orgEmailValidator: 'true' }, resolve));
    } else {
      await new Promise((resolve) =>
        this.setState({ orgEmailValidator: 'false' }, resolve));
    }
  };
  ...
  generalMessage = (inputString: string): ReactElement<{}> => {
    ...
    if (inputString === 'false') {
      return <div className="invalid-feedback">Invalid or Blank field.</div>;
    }
    return <div />;
  };
  ...
  handleStepComplete = async (e) => {
    ...
    await Promise.all([
      ...
      this.validateOrgWebsite(),
      this.validateEIN(),
      this.validateOrgAddress(),
      this.validateOrgCity(),
      this.validateOrgState(),
      this.validateOrgZipcode(),
      this.validateOrgPhoneNumber(),
      this.validateOrgEmail(),
    ]);
    const {
      orgNameValidator,
      orgWebsiteValidator,
    ...
    const { handleContinue, alert } = this.props;
    // check if all elements are valid
    if (
      orgNameValidator === 'true' &&
        ...
        <div className="d-flex justify-content-center pt-5">
          <div className="col-md-10">
            <div className="text-center pb-4 mb-2">
              <h2>
                <b>Next, tell us about your organization.</b>
              </h2>
            </div>
            <form onSubmit={this.handleStepComplete}>
              ...
              <div className="form-group row">
                <label
                  htmlFor="city"
                  className="col-sm-3 col-form-label invisible"
                />
                <div className="col-sm-3 mb-3 mb-sm-0">
                  <label htmlFor="city" className="sr-only sr-only-focusable">
                    City
                  </label>
                  <input
                    type="text"
                ...
                <div className="col-sm-3 mb-3 mb-sm-0">
                  <label htmlFor="state" className="sr-only sr-only-focusable">
                    State
                  </label>
                  <select
                    className={`form-control form-purple ${this.colorToggle(
                  ...
                  >
                    {USStates.map((USState) => (
                      <option key={USState.abbreviation}>
