...
function BaseCardFixture() {
  ...
  const [cardSize] = useSelect('cardSize', {
    options:
      Object.values(CardSize),
  });
  const [image] = useValue('image', { defaultValue: 'https://upload.wikimedia.org/wikipedia/en/5/5f/Original_Doge_meme.jpg' });
  const [imageLoc] = useSelect('imageLoc', {
    options:
      Object.values(CardImageLoc),
  });
