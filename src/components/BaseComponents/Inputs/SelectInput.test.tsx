...
describe('SelectInput', () => {
  ...
  test('should render options in list on click and call onChange with selected value', async () => {
    ...
    const { getByLabelText, getByText } = render(
      ...
      />,
      {
        queries: {
          ...queries,
          queryByNameAttribute: queryByNameAttributeQuery,
        },
      },
    );
  ...
  test('should render the default value', () => {
    ...
    const { queryByNameAttribute, getByText } = render(
      <SelectInput
        label={label}
        name={name}
        options={options}
    ...
    expect(getByText(options[3].label)).toBeInTheDocument();
  });
});
