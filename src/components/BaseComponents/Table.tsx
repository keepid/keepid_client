...
function EditFormatter(props: FormatterProps): React.ReactElement {
  ...
  return (
    <Button
      variant="link"
      className={canEdit ? 'save-text table-button action' : 'edit-text table-button action'}
      onClick={(e) => (canEdit ? handleSave(e) : handleEdit(e))}
    >
  ...
  return (
    <Modal key="deleteRow" show>
      ...
      <Modal.Footer>
        ...
        <button type="button" className="btn btn-primary" data-dismiss="modal" onClick={props.handleClickClose}>
          Cancel
        </button>
      </Modal.Footer>
    </Modal>
            ...
            <div>
              ...
              {showDeleteModal ? (
                <TModal
                  ...
                  handleDelete={(e) => this.handleDelete(e, rowToDelete)}
                />
              ) : null}
            </div>
          )
